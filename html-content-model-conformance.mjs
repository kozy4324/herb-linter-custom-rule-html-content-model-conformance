...
class ContentModelConformanceVisitor extends BaseRuleVisitor {
  ...
  resolveTransparentContentModel() {
    let index = -1;
    for (const tagName of this.elementStack.slice(0, -1).reverse()) {
      const spec = specs[tagName];
      const contentModel = spec?.computedContentModel ? spec.computedContentModel(this.elementStack.slice(0, index)) : spec?.contentModel;
  ...
  handleERBNode(tagOpening, content, location, visitChildren) {
    if (tagOpening !== "<%=") {
      visitChildren();
      return;
    }
  ...
  addOffenseMessage(tagName, parentTagName, location) {
    this.addOffense(
      `Element \`<${tagName}>\` cannot be placed inside element \`<${parentTagName}>\`.`,
      location
    );
  }
  checkConformance(tagName, location) {
    const spec = specs[tagName];
    const parentTagName = this.elementStack.at(-1);
    if (spec && parentTagName && specs[parentTagName]) {
      const check = (spec2, parentContentModel2) => {
        switch (parentContentModel2) {
          case "flow":
          case "phrasing":
            if (!spec2.categories.includes(parentContentModel2)) {
          ...
          case "transparent":
            ...
            if (transparentContentModel) {
              check(spec2, transparentContentModel);
            }
            break;
          case "nothing":
            break;
          case "text":
            break;
          default:
            if (Array.isArray(parentContentModel2)) {
              if (!parentContentModel2.includes(tagName)) {
                this.addOffenseMessage(tagName, parentTagName, location);
              }
            } else {
              const _exhaustiveCheck = parentContentModel2;
              throw new Error(`$unreachable: ${_exhaustiveCheck}`);
            }
        }
      };
      const parentSpec = specs[parentTagName];
      const parentContentModel = parentSpec.computedContentModel ? parentSpec.computedContentModel(this.elementStack.slice(0, -1)) : parentSpec.contentModel;
...
class HTMLContentModelConformanceRule extends ParserRule {
...
const selectElementInnerContentElements = ["option", "optgroup", "hr"].concat(scriptSupportingElements).concat(["noscript", "div"]);
const optgroupElementInnerContentElements = ["option"].concat(scriptSupportingElements).concat(["noscript", "div"]);
const specs = {
  body: {
    categories: [],
    contentModel: "flow",
  ...
  datalist: {
    ...
    get contentModel() {
      return phrasingElements.concat(["option"]).concat(scriptSupportingElements);
    },
...
const flowElements = Object.entries(specs).map(([tag, spec]) => spec.categories.includes("flow") && tag).filter(Boolean);
const phrasingElements = Object.entries(specs).map(([tag, spec]) => spec.categories.includes("phrasing") && tag).filter(Boolean);
const optionElementInnerContentElements = ["div"].concat(phrasingElements).filter((t) => !["datalist", "object"].includes(t));
export {
